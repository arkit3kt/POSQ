...
// POSQ: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
	...
	if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_20_NEW_PROTOCOL_ENFORCEMENT_4) r = SPORK_20_NEW_PROTOCOL_ENFORCEMENT_4_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_20_NEW_PROTOCOL_ENFORCEMENT_4") return SPORK_20_NEW_PROTOCOL_ENFORCEMENT_4;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_20_NEW_PROTOCOL_ENFORCEMENT_4) return "SPORK_20_NEW_PROTOCOL_ENFORCEMENT_4";
